% ==============================================================
% Das Fragedifferential (DFD) -- Version 2.1
...
\hypersetup{
    ...
    linkcolor=blue,
    urlcolor=blue,
    pdftitle={Das Fragedifferential (DFD) -- Version 2.1},
...
\author{
  ...
  Kritischer Dialog: DeepSeek (Seki), Apeiron, Grok, Gemini, Claude, ChatGPT, Manus\\
...
Es gelten die methodologischen Konventionen des OP gem\"{a}\ss{} MKO
(Methodologische Konventionen des OP).
\end{abstract}
...
lokalisierten Geometrie heraus reflektiert. Ohne ein dediziertes Individuationsprinzip
bliebe jeder Monismus eine konturlose Identit\"{a}tsphilosophie, die unf\"{a}hig w\"{a}re,
die ph\"{a}nomenale Vielheit der erlebten Welt zu erkl\"{a}ren.
...
\begin{itemize}
  \item \textbf{Gottfried Wilhelm Leibniz:} Seine \emph{Monadenlehre} l\"{o}st die
        ...
        spiegelt das Universum aus einer spezifischen Perspektive. Das DFD
        formalisiert und dynamisiert diesen Gedanken informationstheoretisch: Die
        Perspektive \emph{k\"{o}nnte} die quantifizierbare informationelle Abweichung
        vom Ganzen sein.
  ...
  \item \textbf{Alfred North Whitehead:} In seiner Prozessphilosophie konstituieren
        sich \glqq{}wirkliche Einzelwesen\grqq{} (actual entities) durch Selektion und
        Abgrenzung (Pr\"{a}hension) von Daten. Das DFD \"{u}bersetzt diesen Gedanken
        in die Sprache einer informationstheoretischen Geometrie --- als Analogie,
  ...
  \item \textbf{Integrierte Informationstheorie (IIT, Tononi):} W\"{a}hrend die IIT
        ...
        werden --- eine Analogie, die der formalen Ausarbeitung durch die PST bedarf.
\end{itemize}
...
ein lokalisierter Fokus im primordialen Feld zu sein, der durch eine pr\"{a}zise
...
($D_{\mathrm{KL}}(Q_E \| P) \neq D_{\mathrm{KL}}(P \| Q_E)$). Diese Richtung ist im
Rahmen des OP bewusst gew\"{a}hlt: Sie beschreibt den informationellen Abstand der
lokalen Perspektive $Q_E$ gegen\"{u}ber dem vollst\"{a}ndigen Denkfeld $P$. Das DFD
k\"{o}nnte daher als quantitatives Ma\ss{} der \emph{konstitutiven Offenheit} einer
Perspektive interpretiert werden. Die bildhafte Formulierung der \glqq{}offenen
Fragen\grqq{} ist keine mathematische Definition, sondern eine philosophische
Interpretation dieser strukturellen Differenz.
\end{remark}
...
\begin{enumerate}
  \item \textbf{Vollst\"{a}ndige Identit\"{a}t ($\mathcal{D}(E) = 0$):} Wenn die
        ...
        Das bedeutet das Ende der Individuation. Absolute Objektivit\"{a}t heben den
        Beobachter auf; das System w\"{a}re mit dem ungeteilten Gesamtfeld identisch.
        Dieser Grenzfall ist --- wie in EG-1 und MKO ausgef\"{u}hrt --- strukturell
        ausgeschlossen f\"{u}r jede endliche Perspektive.
  ...
  \item \textbf{Maximale Isolation ($\mathcal{D}(E) \to \infty$):} Je gr\"{o}\ss{}er
        ...
        erfahren. Dieser Grenzfall ist ebenfalls konzeptuell, nicht empirisch behauptet.
\end{enumerate}
...
einen dogmatischen Absolutismus zu verfallen. Dies gelingt \"{u}ber eine strikte
begriffliche Triade (vgl.\ EG-1, Sektion~7):
...
\begin{itemize}
  \item \textbf{OAD (Ontologische Attraktoren-Dynamik):} Die OAD liefert die stabilen
        Attraktorencluster, deren informationelle Differenz zum Gesamtfeld das DFD
        beschreibt.
  ...
  \item \textbf{EG-1 (Der Explanatory Gap):} Das DFD liefert dort das
        Fragedifferential als Individuationsprinzip und begr\"{u}ndet, warum der
        Explanatory Gap keine schlie\ss{}bare L\"{u}cke ist, sondern die konstitutive
...
\begin{itemize}
  \item \textbf{Analogie: Subjektiver Zeitpfeil.} Warum vergeht Zeit f\"{u}r ein
        Individuum linear? Eine m\"{o}gliche Interpretation: Weil das System durch seine
        Begrenzung gezwungen sein k\"{o}nnte, die raumzeitlose Zustandsverteilung des
        OPP sequenziell zu approximieren. Das Vergehen der Zeit w\"{a}re dann analog
        zur Bewegung des Fragedifferentials im Versuch, sich asymptotisch dem Ganzen
        anzun\"{a}hern.

  \item \textbf{Hypothese: Lernen und lokale Entropieminderung.} Falls der OP zutrifft,
        w\"{a}re zu erwarten, dass jeder Akt des Lernens das Fragedifferential lokal
        verringert ($\Delta \mathcal{D}(E) < 0$). Dies k\"{o}nnte mit einer lokalen
        Entropieminderung der perspektivischen Struktur korrespondieren, analog zum
        zweiten Hauptsatz der Thermodynamik. Ob und wie das empirisch pr\"{u}fbar ist,
        bleibt eine offene Forschungsfrage der PST.
\end{itemize}
...
Das DFD steckt das mathematische Arbeitsprogramm f\"{u}r die PST ab:

\begin{enumerate}
  \item Welche verallgemeinerte informationstheoretische Metrik --- etwa Von-Neumann-Entropie
        auf Operatoralgebren oder eine andere raumzeitlose Struktur --- beschreibt das
        Feld pr\"{a}zise?
